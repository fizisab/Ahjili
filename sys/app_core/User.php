...
class User extends Generic{
	...
	// My Code
	// check if a user by Phone exits
	public static function userPhoneExists($email) {
		$user_id = self::$db->where('phone_number', self::secure($email))->getValue(T_USERS, 'user_id');
		return (empty($user_id)) ? false : true;
	}
	...
	public static function loginUser(){
        ...
        $getUserPassword = self::$db->where("(username = ? or email = ? or phone_number = ?)", array(
            $username,
            $username,
            $username
        ...
        self::$db->where("(username = ? or email = ? or phone_number = ?)", array(
            $username,
            $username,
            $username
        ));
        if (strlen($getUserPassword) == 40) {
        	...
        	if ($validate_password) {
        		$login = self::$db->where("(username = ? or email = ? or phone_number = ?)", array(
		            $username,
		            $username,
            $username
		        ))->getOne(T_USERS);
